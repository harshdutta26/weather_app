...
app.get("/api/timenode", async function(req,res){
   try { const inputval=req.query;
    // const lUrl = `http://api.openweathermap.org/geo/1.0/direct?q=${inputval}&limit=1&appid=40f4f989dc2ca4888010ed73d0c78e8a`;
    const lUrl = `http://api.openweathermap.org/geo/1.0/direct?q=${inputval}&limit=1&appid=${api_key}`;
    const lresponse=await fetch(lUrl);
    const ldata=await lresponse.json();

    // const { lat, long } = req.query;
    const apiUrl = `http://api.timezonedb.com/v2.1/get-time-zone?key=${timeapi_key}&format=xml&by=position&lat=${lat}&lng=${long}`;
